...
public:
  ...
  bool cbPlanToJoint(planning_exec_interface::MoveToJoint::Request &req,
    ...
    ROS_DEBUG_STREAM("planning_group_: " << planning_group_);
...
int main(int argc, char **argv){
  ...
  for(int i=0; i<num_groups; i++){
    ...
    ros::ServiceServer srv =
        nh_move.advertiseService(name_service,
                                 &PlanningExecutionGroup::cbMoveToJoint,
                                 &planning_exec_groups[i]);
  ...
  for(int i=0; i<num_groups; i++){
    ...
    ros::ServiceServer srv =
        nh_move.advertiseService(name_service,
                                 &PlanningExecutionGroup::cbPlanToCartRPY,
                                 &planning_exec_groups[i]);
  ...
  for(int i=0; i<num_groups; i++){
    ...
    ros::ServiceServer srv =
        nh_move.advertiseService(name_service,
                                 &PlanningExecutionGroup::cbPlanToJoint,
                                 &planning_exec_groups[i]);
  ...
  for(int i=0; i<num_groups; i++){
    ...
    ros::ServiceServer srv =
        nh_move.advertiseService(name_service,
                                 &PlanningExecutionGroup::cbExecutePlan,
                                 &planning_exec_groups[i]);
